...
def add_expense(amount, category, desc):
    """Validate inputs; actual storage happens in app.py"""
    try:
...
def budget_analyzer(expenses: List[Dict]):
...
# langchain tools
@tool
def get_budget_summary(expenses: List[Dict]) -> str:
    """Tool 1: my budget analysis tool"""
    return budget_analyzer(expenses)
...
def savings_advisor(expenses):
    if not expenses:
        return "No expenses yet. Add some first."

    # call tools
    budget_info = get_budget_summary.invoke({"expenses": expenses}) or budget_analyzer(expenses)
